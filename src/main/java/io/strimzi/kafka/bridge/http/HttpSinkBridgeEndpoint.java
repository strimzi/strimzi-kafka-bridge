...
import io.strimzi.kafka.bridge.converter.JsonDecodeException;
import io.strimzi.kafka.bridge.converter.JsonUtils;
import io.strimzi.kafka.bridge.http.beans.AssignedTopicPartitions;
...
import io.strimzi.kafka.bridge.http.beans.OffsetCommitSeek;
...
public class HttpSinkBridgeEndpoint<K, V> extends HttpBridgeEndpoint {
    ...
    public HttpSinkBridgeEndpoint(BridgeConfig bridgeConfig, KafkaConfig kafkaConfig, HttpBridgeContext<K, V> context, EmbeddedFormat format,
     ...
     * @return a CompletionStage bringing the CreatedConsumer to send back to the client
     */
    public CompletionStage<CreatedConsumer> createConsumer(UriInfo uriInfo, HttpHeaders httpHeaders, String groupId, Consumer consumerData, Handler<HttpBridgeEndpoint> handler) {
     ...
     * @return a CompletionStage bringing no body to send back to the client
     */
     ...
     * @return a CompletionStage bringing the list of ConsumerRecord to send back to the client
     */
    ...
    private List<ConsumerRecord> pollHandler(ConsumerRecords<K, V> records, Throwable ex) {
        if (ex == null) {
            ...
            try {
                List<ConsumerRecord> messages = messageConverter.toHttpRecords(records);
     ...
     *
     * @return a CompletionStage bringing the list of SubscribedTopicList to send back to the client
     */
    ...
    private Error handleError(Throwable ex) {
        ...
        return HttpUtils.toError(code, ex.getMessage());
    }
