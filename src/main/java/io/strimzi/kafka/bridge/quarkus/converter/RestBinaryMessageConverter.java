 ...
 */
public class RestBinaryMessageConverter implements RestMessageConverter<byte[], byte[]> {
    ...
    public org.apache.kafka.clients.producer.ProducerRecord<byte[], byte[]> toKafkaRecord(String kafkaTopic, Integer partition, ProducerRecord message) {
        ...
        if (message != null) {
            if (message.getKey() != null) {
                key = DatatypeConverter.parseBase64Binary(message.getKey().toString());
            }
            if (message.getValue() != null) {
                value = DatatypeConverter.parseBase64Binary(message.getValue().toString());
            }
