...
/**
 * Implementation of a record converter to deal with the "json" embedded data format
 */
public class HttpJsonRecordConverter implements HttpRecordConverter<byte[], byte[]> {
    ...
    public org.apache.kafka.clients.producer.ProducerRecord<byte[], byte[]> toKafkaRecord(String kafkaTopic, Integer partition, ProducerRecord httpRecord) {
        ...
        if (httpRecord != null) {
            if (httpRecord.getKey() != null) {
                key = JsonUtils.objectToBytes(httpRecord.getKey());
            }
            if (httpRecord.getValue() != null) {
                value = JsonUtils.objectToBytes(httpRecord.getValue());
            }
