...
http.host=0.0.0.0
http.port=443 <1>
http.ssl.enable=true <2>
http.ssl.keystore.certificate.location=/etc/ssl/certs/bridge.crt <3>
http.ssl.keystore.key.location=/etc/ssl/private/bridge.key <4>
http.cors.enabled=true <5>
http.cors.allowedOrigins=https://strimzi.io <6>
http.cors.allowedMethods=GET,POST,PUT,DELETE,OPTIONS,PATCH <7>
----
<1> Configure HTTP Bridge to listen on port 443 for TLS encrypted connections.
<2> Set `true` to enable TLS encryption between HTTP clients and the HTTP Bridge.
<3> Location of the keystore file in PEM format.
<4> Location of the private key in PEM format.
<5> Set to `true` to enable CORS.
<6> Comma-separated list of allowed CORS origins. You can use a URL or a Java regular expression.
<7> Comma-separated list of allowed HTTP methods for CORS.
...
|http.port
|HTTP Bridge port. The default is `8080`. If "http.ssl.enable" is set to `true`, then the default is `443`.
