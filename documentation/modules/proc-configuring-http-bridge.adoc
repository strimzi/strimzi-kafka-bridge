...
.Tuning the executor service pool
...
* High throughput with many concurrent requests: Increase `bridge.executor.pool.size` to handle more simultaneous operations.
* Bursty traffic: Increase `bridge.executor.queue.size` to buffer temporary load spikes.
* Resource-constrained environments: Reduce `bridge.executor.pool.size` to limit thread count, but ensure it's high enough to avoid frequent HTTP 503 errors.
...
|The number of threads in the dedicated executor service pool for all Kafka-related asynchronous operations. The default is `max(4, <available CPUs> * 2)`.

|bridge.executor.queue.size
|The maximum number of tasks that can be queued when all executor threads are busy. The default is `1000`. When both the pool and queue are full, new requests are rejected with HTTP 503. The clients should retry.
|====
...
|http.ssl.named.groups
|Comma-separated list of named groups. The default of the underlying JVM version is used.
