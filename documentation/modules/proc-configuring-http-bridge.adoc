...
|bridge.id
|Bridge identifier. This is used as the Kafka client ID for the internal Kafka clients (producer and admin client).
...
|The maximum number of tasks that can be queued when all executor threads are busy. The default is `1000`. When both the pool and queue are full, new requests are rejected with HTTP 503.
|====
