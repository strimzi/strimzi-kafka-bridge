...
CAUTION: The OpenTracing project is now archived, so Strimzi has no support for OpenTracing.
Please use the supported OpenTelemetry instead.
...
----
<1> The property for enabling OpenTelemetry is uncommented.
+
