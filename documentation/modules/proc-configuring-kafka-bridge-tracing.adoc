...
* HTTP clients send HTTP requests to the Kafka Bridge and receive messages from the Kafka Bridge 
...
+
.Additional exporter environment variable when not using Jaeger 
[source,env]
----
