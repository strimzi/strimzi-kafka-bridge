...
[id='proc-creating-http-bridge-consumer-{context}']
= Creating a HTTP Bridge consumer
...
. Create a HTTP Bridge consumer in a new consumer group named `bridge-quickstart-consumer-group`:
+
...
+
If the request is successful, the HTTP Bridge returns the consumer ID (`instance_id`) and base URL (`base_uri`) in the response body, along with a `200` code.
+
.Example response
...
#...
{
  ...
  "base_uri":"http://_<bridge_id>_-bridge-service:8080/consumers/bridge-quickstart-consumer-group/instances/bridge-quickstart-consumer"
}
----
...
Now that you have created a HTTP Bridge consumer, you can  xref:proc-bridge-subscribing-consumer-topics-{context}[subscribe it to topics].

[role="_additional-resources"]
.Additional resources

* xref:createconsumer[POST /consumers/{groupid}]
